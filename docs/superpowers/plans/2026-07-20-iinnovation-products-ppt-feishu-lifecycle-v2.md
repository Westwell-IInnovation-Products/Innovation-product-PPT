# IInnovation-Products_ppt Feishu Lifecycle V2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Route every actionable IInnovation-Products_ppt review, local automation block, merge result, and release result to Feishu without exposing the Feishu webhook to pull-request code.
...
- Create: `.github/workflows/iinnovation-products-ppt-feishu-notifications.yml`
...
on:
  workflow_run:
    workflows: [IInnovation-Products_ppt Team Sharing, Tag Approved IInnovation-Products_ppt Version, Release IInnovation-Products_ppt Skill]
    types: [completed]
  pull_request_target:
    types: [closed]
  pull_request_review:
    types: [submitted]
  repository_dispatch:
    types: [iinnovation_products_ppt_local_alert]
  workflow_dispatch:
permissions:
  actions: read
  contents: read
...
Delete both `Notify Feishu...` steps from `iinnovation-products-ppt-team-sharing.yml`, leaving validation and rendering unchanged and secret-free.
...
The script must allow only `candidate-cycle-blocked`, `consumer-update-failed`, and `automation-disabled`; derive `owner/repo` from `origin`; obtain the existing Git credential without printing it; POST `event_type=iinnovation_products_ppt_local_alert`; and support `--dry-run` before credential lookup.
...
try {
  ...
  Send-TeamAlert 'candidate-cycle-blocked' '候选处理被安全规则阻断' '请检查本机 IInnovation-Products_ppt 审计日志。'
  throw
}
```
...
```powershell
git add -- .github team-sharing docs/governance docs/superpowers/plans/2026-07-20-iinnovation-products-ppt-feishu-lifecycle-v2.md
...
```powershell
git push -u origin agent/iinnovation-products-ppt-feishu-lifecycle-v2
```
