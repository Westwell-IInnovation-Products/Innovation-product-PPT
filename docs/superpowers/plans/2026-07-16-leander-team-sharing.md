# Leander Team Sharing Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and prove a zero-extra-software workflow in which multiple people submit isolated Leander PPT component candidates, a curator promotes approved candidates into the executable component library, GitHub validates the result, and teammates install a stable release.

**Architecture:** GitHub `main` is the canonical source. Contributors submit one candidate bundle per branch under `contributions/leander-ppt/components/<github-user>/<candidate-id>/`; candidate branches never overwrite the installed Skill. A curator promotion command validates the bundle, copies its renderer into the extension runtime, adds a reviewed registry entry, rebuilds the compact index, and runs Leander component gates. Local Git and PowerShell handle branch/push operations; a Node script uses the existing Git credential helper to create Draft PRs through GitHub REST, so GitHub CLI is not required.

**Tech Stack:** Git, PowerShell, Node.js built-ins, Leander PPT Node toolchain, GitHub Actions, GitHub REST API.
...
- `.github/workflows/leander-team-sharing.yml`: runs candidate tests, Skill hygiene, registry/index consistency, component lint, and dual-theme preview checks.
- `.github/workflows/leander-release.yml`: validates `leander-ppt-v*` tags and publishes the clean Skill archive.
- `docs/governance/leander-team-sharing.md`: team policy, roles, change classes, review gates, release and rollback process.
...
- `team-sharing/scripts/install-leander.ps1`: installs a validated repository/tag copy into the user's Codex Skill directory with rollback backup.
...
- `leander-ppt/templates/leander-ppt-scaffold/components/extensions/index.js`: loads promoted extension renderer modules.
- `leander-ppt/templates/leander-ppt-scaffold/tools/component-runtime.js`: includes promoted extensions in renderer discovery.
- `leander-ppt/templates/leander-ppt-scaffold/tools/render-component-library-preview.js`: renders through the unified component runtime.
- `leander-ppt/CHANGELOG.md`: release history beginning with the shared beta baseline.

### Task 1: Establish the clean Skill baseline

- [ ] Mirror `C:\Users\admin\.codex\skills\leander-ppt` into the repository `leander-ppt/` on `agent/leander-team-sharing-pilot`.
- [ ] Run `node scripts/release-hygiene.js` from `leander-ppt` and expect `PASS release hygiene`.
- [ ] Run `node templates/leander-ppt-scaffold/tools/lint-scope-hygiene.js --skill-root .` and expect `PASS scope hygiene`.
- [ ] Add `leander-ppt/CHANGELOG.md` with `0.6.0-beta.8` as the initial governed baseline.
...
- [ ] Implement `install-leander.ps1` with source validation, backup, mirror copy, and rollback on failed post-install hygiene.
...
- [ ] Add `leander-team-sharing.yml` for Node tests, candidate validation, release hygiene, scope hygiene, index consistency, strict component lint, and dual-theme previews.
- [ ] Add `leander-release.yml` for `leander-ppt-v*` tags and a clean release archive.
...
- [ ] Tag `leander-ppt-sim-v0.1.0`, clone as a consumer, run the installer, and confirm both component IDs are installed.
...
- [ ] Run `node team-sharing/scripts/validate-candidate.js contributions/leander-ppt/components`.
- [ ] Run Leander release and scope hygiene.
...
- [ ] Push `agent/leander-team-sharing-pilot` using local Git.
