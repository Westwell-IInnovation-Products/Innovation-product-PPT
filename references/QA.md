...
- [ ] 正文和卡片文字可读。
- [ ] 长正文没有批量使用 6–7pt 的 `tiny/micro`；注释级字号只承担短标签、图例、来源和图内说明。
- [ ] 页面没有意外偏向一侧。
- [ ] 留白是设计出来的,而不是空的。
- [ ] 高卡片内部没有“顶部堆字、底部孤立标签、中间大面积空洞”的断裂结构。
...
- **注释字号承载正文**:正文区域出现多段长文字使用低于 7.5pt 的字号，或 `tiny/micro` 成为主要阅读层。若确属高密度工程表，必须在 `geometryPolicy.typography` 中说明原因并由全尺寸评审验证。
- **卡内纵向空洞**:高卡片顶部集中标题和说明、底部孤立标签，中间留出超过卡高约 40% 的无内容间隔。
- **rail 语义误用**:角色、职责、类别或名称与 review 有关，但并没有真实状态转换时仍使用 review rail。
