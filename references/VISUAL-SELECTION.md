...
这个关卡把“凭感觉画一页”变成可审计的表达路线选择：

```text
页面意图 -> 蓝图边界 -> 基本关系类型 -> 必需槽位 -> 内容容量 -> 证据/主题适配 -> 风险淘汰 -> 路线绑定 -> 渲染复核
```
...
蓝图已经确定页面版面结构、表达模式和搜索边界，选择器只负责在边界内找适配实现，不重新设计页面。
...
| 维度 | 作用 | 典型处理 |
|---|---|---|
| 蓝图边界 | 是否属于精确 `candidateComponents` 和允许路线 | 白名单 + 强排序；无效 ID 直接失败 |
| 基本关系类型 | 是否表达同一种逻辑关系 | 主分 |
...
| `patternHints` / 关键词 | 只作为弱信号 | 最多贡献少量分数，不能替代真实组件 ID |

严禁重复加分。候选族和路线偏好只能在一个评分阶段生效。

没有定量证据的数据图组件不得作为机制页、收敛页或流程页的自动 fallback。截图/来源证据主导页应优先竞争 `external-graphic`；锁定路线没有可行候选时直接失败，不再静默改选无关组件。
...
在 render 前做整套选择审计：同一组件被选择 3 次时警告，超过 3 次时默认阻断；确属章节系列或视觉回声时，每个重复页必须填写 `componentReuseRationale` 或在蓝图写 `echoRationale`。同样检查 `previewPattern + primaryShapeClass` 的重复，避免换组件名但继续复用同一几何版面结构。
...
- 选中组件超出蓝图精确候选：必须重新打开蓝图或记录 override 并触发组件管理员，不能静默忽略无效候选。
...
不允许把两个完整 page pattern 硬拼在一页。只需要局部能力时，应拆出局部版块或视觉零件。
...
但声明一致还不够。渲染时 `deck-ctx.js` 会记录真实组件调用到 `out/component-trace.json`；关卡会核对：
