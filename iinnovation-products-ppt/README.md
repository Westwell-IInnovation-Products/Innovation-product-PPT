# IInnovation-Products_ppt

`IInnovation-Products_ppt` 是一套面向正式演示文稿生产的工程化 Skill。它将需求理解、叙事规划、视觉设计、页面实现、渲染评审和终版构建组织为一条可审查、可回溯、可增量修订的生产链，最终输出标准、可编辑的 `.pptx`。

Codex 要求 Skill 内部名称只能使用小写字母、数字和连字符，因此安装目录与调用标识为 `iinnovation-products-ppt`，调用方式为 `$iinnovation-products-ppt`；界面展示名称保持为 `IInnovation-Products_ppt`。
...
- 将 IInnovation-Products_ppt 的流程 Gate 误解为操作系统级安全沙箱。
...
- “使用 iinnovation-products-ppt 将这份项目材料整理为管理层汇报。”
...
IInnovation-Products_ppt 采用“先语义、后结构；先约束、后实现；先代表性验证、后规模化生产”的渐进式流程。
...
IInnovation-Products_ppt 不把 PPT 制作视为一次性生成任务，而是将其视为同时包含内容结构、视觉语法、可执行代码和质量证据的复合型交付过程。
...
IInnovation-Products_ppt 的总体架构可以理解为四层：
...
IInnovation-Products_ppt 采用“专业判断分离、角色按事件触发、主 Agent 保持单一责任”的协作结构。角色是工作流中的评审或决策节点，不是各自独立制作整份 PPT 的平行团队。
...
├── references/
├── scripts/
└── templates/iinnovation-products-ppt-scaffold/
```
...
| `templates/iinnovation-products-ppt-scaffold/` | 新 Deck 的可执行项目模板 |
...
| 强制入口与路由 | `SKILL.md` |
| 可执行行为 | `templates/iinnovation-products-ppt-scaffold/tools/*.js` |
| 主题外观 | `theme/` 与主题感知组件 |
...
```powershell
node .\templates\iinnovation-products-ppt-scaffold\tools\build-component-index.js
node .\templates\iinnovation-products-ppt-scaffold\tools\lint-component-library.js --strict
```
