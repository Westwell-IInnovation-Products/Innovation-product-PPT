...
这份文件定义 IInnovation-Products_ppt 如何把用户反馈转化为可复用规则，并防止问题越积越多、token 越用越重。
...
- 需要积累的规则已写入对应位置。
- 不需要积累的问题已明确留在项目内，不污染公共 skill。

## 可执行生命周期

项目框架使用 `state/issues.json` 保存结构化问题，并通过以下命令维护：
...
2. 已抽象为页面关系、表达模式、组件能力、QA 风险或流程关卡。
