# IInnovation-Products_ppt 框架

框架是那个可复用的工作结构,它防止每个 PPT 任务都沦为一次性脚本。

## "真正的框架"意味着什么

对 IInnovation-Products_ppt 来说,真正的框架不只是指令。它是一套文件夹结构、主题 token、组件 helper、抽取工具和 QA 工具,可以跨 deck 复用。

可执行的 Codex 框架随 skill 发布在 `templates/iinnovation-products-ppt-scaffold/`。它包含主题 token、组件库、图标 helper、逐页生产工具,以及输出目录。
