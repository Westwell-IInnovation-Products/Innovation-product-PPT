...
function loadExtensions(context) {
  ...
  for (const file of files) {
    ...
    if (!extension || typeof extension.name !== "string" || typeof extension.create !== "function") {
      throw new Error(`Invalid IInnovation-Products_ppt extension module: ${file}`);
    }
    if (Object.prototype.hasOwnProperty.call(renderers, extension.name)) {
      throw new Error(`Duplicate IInnovation-Products_ppt extension renderer: ${extension.name}`);
    }
    const renderer = extension.create(context);
    if (typeof renderer !== "function") {
      throw new Error(`IInnovation-Products_ppt extension ${extension.name} did not create a renderer function.`);
    }
