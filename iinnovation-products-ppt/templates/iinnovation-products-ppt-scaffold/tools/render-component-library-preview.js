...
function sampleFor(c) {
  ...
  switch (c.name) {
    ...
    case "hubSpokeCapability":
      return { ...d, center: "IInnovation-Products_ppt", takeaway: "One harness combines routing, components, QA, and learning.", modules: items4.concat({ title: "Learning", desc: "promote lessons", icon: "chart" }).map((x, i) => ({ title: x.title, desc: x.desc, icon: x.icon || "document", status: i === 1 ? "key" : "" })) };
    case "hubRadial":
    ...
    case "quoteHighlight":
      return { ...d, quote: [{ text: "A component is not a page meaning; " }, { text: "it is a reusable relationship.", hot: true }], by: "IInnovation-Products_ppt component principle" };
    case "numberedList":
...
async function main() {
  ...
  pptx.author = "IInnovation-Products_ppt";
  pptx.company = "Westwell";
  pptx.subject = "Real component library preview";
  pptx.title = "IInnovation-Products_ppt Real Component Preview";
  pptx.lang = "en-US";
  const manifest = [];
