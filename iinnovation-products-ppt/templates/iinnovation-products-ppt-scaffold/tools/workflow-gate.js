// Mandatory workflow entry and stage gate for every IInnovation-Products_ppt deck.
// Usage:
...
function initialize(intent) {
  ...
  if (deltaCarry) {
    ...
    console.log(`IInnovation-Products_ppt Gate 0 (delta-revision) initialized: ${receipt.runId}`);
    ...
    console.log(`IInnovation-Products_ppt Gate 0 initialized: ${receipt.runId}`);
...
function migrateExisting(stage, note, receiptDir, explicitRunId) {
  ...
  console.log(`Migrated existing IInnovation-Products_ppt project at ${stage}: ${receipt.runId}`);
}
...
function verify(target) {
  ...
  console.log(`IInnovation-Products_ppt workflow gate OK: ${target} (${receipt.runId})`);
}
