...
try {
  ...
  if (!inside(path.join(repo, "contributions", "leander-ppt", "components"), candidate)) throw new Error("Promotion candidate must be inside the governed contributions area.");
  ...
  const scaffold = path.join(repo, "leander-ppt", "templates", "leander-ppt-scaffold");
  ...
  try {
    npmCi(scaffold);
    run(process.execPath, [path.join(repo, "team-sharing", "scripts", "promote-candidate.js"), candidate, "--skill-root", path.join(repo, "leander-ppt"), "--curator", curator, "--approve-production"], repo, { echo: true });
  } finally {
    fs.rmSync(nodeModules, { recursive: true, force: true });
    fs.rmSync(lintEvidence, { force: true });
  }
  run(process.execPath, [path.join(repo, "team-sharing", "scripts", "bump-leander-version.js"), "--root", path.join(repo, "leander-ppt"), "--type", releaseType, "--preid", preid, "--note", `Promote reviewed component ${metadata.id}.`], repo, { echo: true });
  const preparedVersion = JSON.parse(fs.readFileSync(path.join(repo, "leander-ppt", "manifest.json"), "utf8")).version;
  run(process.execPath, [path.join(repo, "leander-ppt", "scripts", "release-hygiene.js")], path.join(repo, "leander-ppt"), { echo: true });
  git(repo, "add", "--", "leander-ppt", "contributions/leander-ppt/promotions");
  git(repo, "commit", "-m", `Promote Leander component ${metadata.id}`);
  ...
  if (process.argv.includes("--create-draft-pr")) {
    ...
    const output = run(process.execPath, [path.join(repo, "team-sharing", "scripts", "create-draft-pr.js"), "--repo", match[1], "--head", branch, "--base", "main", "--title", `Promote Leander component ${metadata.id}`, "--body-file", path.join(repo, ".github", "pull_request_template.md")], repo);
