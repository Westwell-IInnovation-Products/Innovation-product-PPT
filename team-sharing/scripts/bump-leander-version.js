...
try {
  const root = path.resolve(value("root", "leander-ppt"));
  const type = value("type");
  const preid = value("preid");
  const note = value("note", "Automated approved team-sharing update.").replace(/[\r\n]+/g, " ").trim();
  const manifestFile = path.join(root, "manifest.json");
  const packageFile = path.join(root, "templates", "leander-ppt-scaffold", "package.json");
  const lockFile = path.join(root, "templates", "leander-ppt-scaffold", "package-lock.json");
  const scaffoldFile = path.join(root, "templates", "leander-ppt-scaffold", ".leander-scaffold-version.json");
  const deckFile = path.join(root, "templates", "leander-ppt-scaffold", "tools", "deck.js");
  ...
  console.log(JSON.stringify({ previous, version: next, tag: `leander-ppt-v${next}` }));
