...
async function sendFeishu(payload, webhook = process.env.FEISHU_WEBHOOK_URL, options = {}) {
  if (!webhook) {
    if (options.required) throw new Error("FEISHU_WEBHOOK_URL is not configured");
    return { skipped: true };
  }
...
async function main() {
  ...
  const sent = await sendFeishu(payload);
  if (sent.skipped) { console.log("SKIP Feishu notification: FEISHU_WEBHOOK_URL is not configured"); return; }
