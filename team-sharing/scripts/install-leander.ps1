...
$source = Join-Path $repo 'leander-ppt'
if (-not (Test-Path -LiteralPath (Join-Path $source 'SKILL.md'))) { throw "Leander Skill not found under RepositoryRoot." }
...
& node (Join-Path $source 'templates\leander-ppt-scaffold\tools\lint-scope-hygiene.js') --skill-root $source
if ($LASTEXITCODE -ne 0) { throw "Source scope hygiene failed." }

$parent = Split-Path $destinationFull -Parent
New-Item -ItemType Directory -Force -Path $parent | Out-Null
$staging = Join-Path $parent ('.leander-ppt.installing.' + [guid]::NewGuid().ToString('N'))
$backup = $null
...
Write-Output "INSTALLED_LEANDER_VERSION=$($manifest.version)"
