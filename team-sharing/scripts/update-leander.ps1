...
if ($LASTEXITCODE -ne 0) { throw 'Unable to fetch Leander release tags.' }
$tags = & git -C $repo tag -l 'leander-ppt-v*'
if ($LASTEXITCODE -ne 0) { throw 'Unable to list Leander release tags.' }
$current = $null
...
try {
  ...
  $releaseManifest = Get-Content -Raw -LiteralPath (Join-Path $worktreeFull 'leander-ppt\manifest.json') | ConvertFrom-Json
  if ($releaseManifest.version -ne $selection.selectedVersion) { throw 'Release tag and manifest version do not match.' }
  $installer = Join-Path $worktreeFull 'team-sharing\scripts\install-leander.ps1'
  & $installer -RepositoryRoot $worktreeFull -Destination $Destination
  if ($LASTEXITCODE -ne 0) { throw 'Leander installer failed.' }
