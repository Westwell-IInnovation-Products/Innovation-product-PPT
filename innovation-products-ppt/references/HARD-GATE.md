...
Innovation-Products_ppt 的 Gate 是一组 fail-closed 的流程边界,不是操作系统级的安全沙箱。它约束用户审批、修订范围、来源、运行环境、独立评审与最终产物。(按 token 拆任务/轮换的预算门禁已在单任务模式下退休;Token 账本只度量成本。用户审批与质量门等**该停的地方照旧停**。)
...
本约束覆盖强制的 Innovation-Products_ppt 入口:`workflow-gate.js`、非 status 的 `run-phase.js` 命令,以及 `deck.js render|verify|build`。终版交付必须来自 `deck.js`,并带有效的 workflow/approval/agent/final-artifact receipts、来源证据、当前环境指纹和 QA 证据。

本约束无法阻止一个拥有不受限文件系统访问权的进程去删除 state 文件、改动 Skill 代码、直接编辑页面源码,或另建一条不相干的 PPTX 流水线。那些行为都是流程绕过,会使 Innovation-Products_ppt 交付失效;一个本地 Node Skill 不是 OS 沙箱,也无法强制 Codex 宿主 UI 终止或创建新任务。approval/agent receipt 中的 opaque Codex ID 必须来自宿主工具结果；如果宿主没有 lookup API,本地 Skill 能证明 ID、消息/事件和文件之间的完整绑定,但不能独立证明该 ID 在宿主真实存在。
