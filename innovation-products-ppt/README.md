...
3. 解压后找到 `innovation-products-ppt` 文件夹。
...
也可以在已克隆的 `Leander` 仓库根目录运行以下 PowerShell。该命令会先备份旧版本，再复制新版本：

```powershell
$repoRoot = (Resolve-Path .).Path
$skillSource = Join-Path $repoRoot "innovation-products-ppt"
...
ZIP 用户重新下载仓库，并用新文件夹替换本地安装目录。Git 用户先在仓库中执行：
