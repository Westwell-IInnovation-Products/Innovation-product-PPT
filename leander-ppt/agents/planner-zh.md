...
7. 对照 `references/QUALITY-BASELINE.md` 检查内容充实度，不能只完成标题和版式字段。
8. 按页面类型补齐最低解释结构：案例讲动作和结果，机制讲输入和输出，框架讲层级和关系，收益讲基线和边界。
...
任何内容页只有观点没有支撑、只有模块名没有职责关系、或只有案例名没有动作结果时，也必须是 `FIX-FIRST`。
