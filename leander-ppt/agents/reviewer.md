...
- The affected `pages/<id>/page.json` files, especially `visualSelection`.
- The project `role-briefs.md` section for reviewer/review scope, if present.
- The project `visual-direction.md`, if visual polish or "looks wrong" is in scope.
- Read these checklists yourself: `references/QA.md`, `references/SLIDE-CRAFT.md`, `references/VISUAL-SELECTION.md`, `references/LESSONS.md`. They are the bar.
...
7. **Wrong visual route** — `page.json.visualSelection` is missing, the rendered page does not follow the selected route, or the page hand-draws boxes while a close catalog component exists.
8. **Background/logo consistency** — a page whose ground or logo size/position differs from the rest.
9. **Data boundary** — numbers/claims missing achieved/planned/estimate/public-reference labels; invented data/logos.
10. **Template/style fit** — does it match the approved anchor sample and theme? Mismatch is a fail even if nothing clips.

## Visual route review

For every content page, answer:

- What relationship is the page trying to express?
- Does `visualSelection.selectedRoute` match that relationship?
- Did the builder consider component-library, external-graphic, and image2/imageSlot routes before page-specific custom?
- Is a catalog component (`stateFlow`, `workflowConfig`, `pipelineFlow`, `featureGrid`, `hubSpokeCapability`, etc.) a better fit than what was rendered?
- Is the route only a style label such as "line-frame" instead of an actual visual relationship?
...
Visual Route Review:
- p<NN> / selected route / fit verdict / better alternative if any
