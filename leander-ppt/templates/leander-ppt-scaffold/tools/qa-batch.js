...
const { init, verify, writeMarkdown, evidenceError, evidenceSetErrors } = require("./verify-qa-result");
...
function initialize() {
  ...
  for (const dir of dirs()) {
    ...
    const bound = result?.renderSha256 === shaFile(render) && result?.digests?.renderDigest === digests.renderDigest && result?.digests?.selectionOutcomeDigest === digests.selectionOutcomeDigest && result?.digests?.qaDigest === digests.qaDigest && result?.digests?.sourceDigest === digests.sourceDigest;
    if (current.ok) rows.push({ dir, action: "preserved-current-pass" });
    else if (bound) rows.push({ dir, action: "preserved-current-work" });
...
function applyBatch(file) {
  ...
  for (const dir of dirs()) {
    ...
    const mergedChecks = (result.checks || []).map(check => checks[check.ruleId] ? { ...check, ...checks[check.ruleId] } : check);
