# Leander PPT · Git 操作说明

本文写给维护和发布 `leander-ppt` 的同事，覆盖日常更新、分支、提交、Pull Request、版本标签、安装同步和回滚。默认环境为 Windows PowerShell，仓库地址为：
...
Git 中维护的是 `Leander/leander-ppt/`。不要直接把 `%USERPROFILE%\.codex\skills\leander-ppt` 当作共享仓库；安装目录可能包含个人试验或运行痕迹。
...
```powershell
git switch -c release/leander-ppt-team-share
```

后续普通更新可使用：

```powershell
git switch -c docs/leander-ppt-readme
git switch -c fix/leander-ppt-theme-registry
git switch -c feat/leander-ppt-component-name
```
...
- `leander-ppt/` 的完整 Skill。
- 三个公开注册主题：`leander-base`、`base2`、`leander-global`。
- `leander-ppt/docs/theme-samples/` 中三套参考材料和总览图。
...
二进制 PPT 不会显示文本差异，因此还要人工打开 `leander-ppt/docs/theme-samples/` 下的三份 PPT 和三张总览图。
...
```powershell
git commit -m "release(leander-ppt): prepare three-theme team share"
```
...
```powershell
git push -u origin release/leander-ppt-team-share
```

然后在 GitHub 上创建 Pull Request：

- Base：`main`
- Compare：`release/leander-ppt-team-share`
- 标题：`release(leander-ppt): prepare three-theme team share`
...
```powershell
git tag -a leander-ppt-v0.6.0-beta.20-team.1 -m "Leander PPT three-theme team share"
git push origin leander-ppt-v0.6.0-beta.20-team.1
```
...
$skillSource = Join-Path $repoRoot "leander-ppt"
$skillsRoot = Join-Path $env:USERPROFILE ".codex\skills"
$skillTarget = Join-Path $skillsRoot "leander-ppt"
...
```powershell
Test-Path -LiteralPath (Join-Path $env:USERPROFILE ".codex\skills\leander-ppt\SKILL.md")
```
...
```powershell
git restore -- leander-ppt\path\to\file
```
...
1. 关闭 Codex。
2. 将当前 `%USERPROFILE%\.codex\skills\leander-ppt` 改名保留。
3. 将所需的 `leander-ppt.backup-时间戳` 改回 `leander-ppt`。
4. 重启 Codex。
