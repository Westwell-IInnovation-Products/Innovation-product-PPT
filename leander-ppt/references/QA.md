...
- Render-quality evidence: `output/render-quality-evidence.json`, bound to the current contact sheet, page PNG hashes, approved anchor contact sheet, visual-designer report, and reviewer report.

## Render Quality Lock

For a final production deck, rendered QA is not complete until the evidence is captured and independently reviewed:

```bash
node tools/render-contact-sheet.js
node tools/render-quality-gate.js capture
node tools/render-quality-gate.js record --role visual-designer-zh --artifact agent-reviews/visual-designer-zh.md --verdict PASS --pages all --full-size <risk-pages> --addresses all
node tools/render-quality-gate.js record --role reviewer-zh --artifact agent-reviews/reviewer-zh.md --verdict SHIP --pages all --full-size <risk-pages> --overall SHIP
node tools/render-quality-gate.js verify
```

The capture must happen after the current render and contact sheet. It creates `renderSetSha256`; both independent reports include `[render-set:<sha256>]`. Every page gets deterministic gates and current contact-sheet coverage. High-risk pages get full-size review; ordinary pages escalate to full size only on warnings. Evidence V2 keeps page-hash-bound review events, so a single-page repair only re-reviews that page plus the refreshed contact sheet; unchanged pages retain valid coverage. Theme/component changes still require full-deck render and review. `--addresses all` requires literal `[topic:<warning field>]` markers for current quality warnings.

Both independent reports must also include `[quality-score:<0-10>]` and one `[quality:<dimension>=<0-10>]` marker for every dimension in `quality-target.json`. A verdict below the overall or dimension target is `FIX-FIRST`; PASS/SHIP text cannot override the numeric quality target.
...
- [ ] The deck and every content page satisfy `QUALITY-BASELINE.md`; metadata completeness is not accepted as content completeness.
...
- [ ] Case pages state actor, action, result/progress, relevance, and source boundary.
- [ ] Mechanism pages state the original problem, input, mechanism, output/evidence, and limitation or failure path where relevant.
- [ ] No production note, hidden reasoning, or design instruction appears as audience-facing slide copy.
...
7. **Log and classify the lesson.** Append a structured entry to the active deck project's `state/feedback-log.md`, classify it using [`SELF-EVOLUTION.md`](SELF-EVOLUTION.md), and if it recurs or is universal, distill a de-identified rule into [`LESSONS.md`](LESSONS.md), [`DYNAMIC-QA.md`](DYNAMIC-QA.md), component rules, or a process gate. A repair is not complete until logged and classified.
